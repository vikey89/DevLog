...
class YesterdayCommand : CliktCommand(name = "yesterday"), KoinComponent {
    override fun help(context: Context): String = "What you did yesterday across all repos."
    ...
    override fun run() = runBlocking {
        ...
        val report = if (raw) {
            useCase(effectiveConfig, ReportType.Yesterday, rawOnly = true, noCache = noCache)
        } else {
            withSpinner("Generating report...") {
                useCase(effectiveConfig, ReportType.Yesterday, noCache = noCache)
            }
        }

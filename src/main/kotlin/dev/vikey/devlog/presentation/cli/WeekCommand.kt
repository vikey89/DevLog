...
class WeekCommand : CliktCommand(name = "week"), KoinComponent {
    override fun help(context: Context): String = "Weekly summary across all repos."
    ...
    override fun run() = runBlocking {
        ...
        val report = if (raw) {
            useCase(effectiveConfig, ReportType.Weekly, rawOnly = true, noCache = noCache)
        } else {
            withSpinner("Generating report...") {
                useCase(effectiveConfig, ReportType.Weekly, noCache = noCache)
            }
        }

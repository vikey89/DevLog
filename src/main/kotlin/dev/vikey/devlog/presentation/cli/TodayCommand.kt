...
class TodayCommand : CliktCommand(name = "today"), KoinComponent {
    override fun help(context: Context): String = "What you did today across all repos."
    ...
    override fun run() = runBlocking {
        ...
        val report = if (raw) {
            useCase(effectiveConfig, ReportType.Daily, rawOnly = true, noCache = noCache)
        } else {
            withSpinner("Generating report...") {
                useCase(effectiveConfig, ReportType.Daily, noCache = noCache)
            }
        }
